...
void freeInstance(char* fname, fmiComponent c) {
    ...
    if (!comp) return;
    if (comp->loggingOn) comp->logger(c, comp->instanceName, fmiOK, "log", fname);
//    if (comp->r) comp->freeMemory(comp->r);
//    if (comp->i) comp->freeMemory(comp->i);
//    if (comp->b) comp->freeMemory(comp->b);
//    if (comp->s) {
//        int i;
//        for (i=0; i<NUMBER_OF_STRINGS; i++){
//            if (comp->s[i]) comp->freeMemory((void *)comp->s[i]);
//        }
//        comp->functions.freeMemory((void *)comp->s);
//    }
//    if (comp->isPositive) comp->freeMemory(comp->isPositive);
...
fmiStatus fmiSetBoolean(fmiComponent c, const fmiValueReference vr[], size_t nvr, const fmiBoolean value[]){
    ...
    for (i = 0; i < nvr; i++) {
        status = max(status, setBoolean(comp, vr[i], value[i]));
        if (status > Warning) return status;
    }
